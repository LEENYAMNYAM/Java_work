...
//			배열이 가득 차면 반복문 탈출
			...
			System.out.printf("%13s %3d %6s","입력받은 랜덤 수는 : ", ranNum, "입니다.  ");
//			System.out.print("입력받은 랜덤 수는 : " + ranNum + "입니다.");
...
//			랜덤수와 같은 수가 있으면 다시 입력(while문 다시 시작)
			if(tmp==ranNum) {
				tmp=-999;
				System.out.println(ranNum +"은 중복된 수입니다. 다시 랜덤수를 출력합니다.");
			}else {
//				3의 배수인지 확인
				if(ranNum%3==0) {
//					3의 배수이면 배열에 추가하고 count를 1증가 시킴.
					System.out.println(ranNum + "은 3의 배수이므로, 배열에 추가합니다.");
					DB[count++]=ranNum;
				}else {
					System.out.println(ranNum + "은 3의 배수가 아니므로, 다시 랜덤수를 출력합니다.");
